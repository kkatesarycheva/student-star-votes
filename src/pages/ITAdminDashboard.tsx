...
const ITAdminDashboard = () => {
  ...
  if (!isLoggedIn || !isITAdmin) { navigate("/login"); return null; }
  ...
  const handlePhotoUpload = () => {
    ...
    setTimeout(() => {
      ...
      toast.success(`${photoFiles.length} photo(s) uploaded! (Demo — no actual storage without backend)`);
    }, 1000);
  };
        ...
        <Tabs defaultValue="manage" className="space-y-6">
          ...
          <TabsContent value="photos">
            <div className="bg-card border border-border rounded-xl p-6 shadow-card space-y-6">
              ...
              {photoFiles.length > 0 && (
                <div className="flex items-center gap-3">
                  <Button onClick={handlePhotoUpload} className="bg-gradient-gold text-secondary-foreground font-semibold hover:opacity-90 gap-2">
